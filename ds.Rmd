...
Then I extracted job skills required for each job. In order to do that, I created a list of job skills that are most commonly used in Data Scientist Job, including hadoop, python, sql, r, spark,sas,aws,excel,azure,java,tableau, deep learning, machine learning, AI,pandas, scipy, c,perl,text mining, matlab,hive,splunk, data mining, time series, jupyter, shell, unix/linex, cloud, apache, and swift. Then I used grepl to find a pattern match in each job description, I counted each skill once if a match is found. By doing this iterating over all jobs, I obtained a total count of each skill among all jobs.The following barplot shows the counts for each skill in descending order for all the job listings. Surprisingly, SQL turns out to be the skill with most counts, following by pyhon, java, machine learning, R, and so on. 
...
skills=data.frame(hadoop=as.numeric(sapply(full_dat$desc,function(x) any(grepl("\\sHadoop\\b",x, ignore.case=TRUE)))),
          ...
          deep_learning=as.numeric(sapply(full_dat$desc,function(x) any(grepl("deep.learning",x, ignore.case=TRUE)))),
...
time_series=as.numeric(sapply(full_dat$desc,function(x) any(grepl("time.series",x, ignore.case=TRUE)))),
jupyter=as.numeric(sapply(full_dat$desc,function(x) any(grepl("\\sjupyter\\b",x, ignore.case=TRUE)))),
shell=as.numeric(sapply(full_dat$desc,function(x) any(grepl("\\sshell\\b",x, ignore.case=TRUE)))),
unix_linux=as.numeric(sapply(full_dat$desc,function(x) any(grepl(or(SPC%R%"unix"%R%SPC,SPC%R%"linux"%R%SPC),x, ignore.case=TRUE)))),
cloud=as.numeric(sapply(full_dat$desc,function(x) any(grepl("\\scloud\\b",x, ignore.case=TRUE)))),
apache=as.numeric(sapply(full_dat$desc,function(x) any(grepl("\\sapache\\b",x, ignore.case=TRUE)))),
swift=as.numeric(sapply(full_dat$desc,function(x) any(grepl("\\sswift\\b",x, ignore.case=TRUE))))
)
...
prop2=as.numeric(apply(full_dat[full_dat$ind==1,7:37], 2, function(x) sum(x,na.rm = T)/dim(full_dat[full_dat$ind==1,])[1]))
...
prop3=as.numeric(apply(full_dat[full_dat$ind==2,7:37], 2, function(x) sum(x,na.rm = T)/dim(full_dat[full_dat$ind==2,])[1]))
...
```{r echo=FALSE,message=FALSE,warning=FALSE}
#by industry
skills_by_industry=as.data.frame(full_dat[,6:37]%>%group_by(industry_clean)%>%summarise_each(funs(mean(., na.rm=TRUE))))
