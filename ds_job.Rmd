...
page.links <- indeed %>%
  ...
  html_attr('href')
indeed_results=html_nodes(indeed,".result")

html_tmp=lapply(indeed_results,function(x) ifelse(length(html_nodes(x,"h2 a"))==0,"No html",html_nodes(x,"h2 a")%>%html_attr('href')))
html=ifelse(html_tmp=="No html","No html",paste0(base,html_tmp))

indeed_results_tmp=strsplit(html_text(indeed_results),"\n\n")
indeed_results=data.frame(
  title=unlist(lapply(indeed_results_tmp, function(x) x[2])),
  company=gsub("\n","",unlist(lapply(indeed_results_tmp, function(x) ifelse(x[3]=="",x[4],x[3])))),
  experience=unlist(lapply(indeed_results_tmp, function(x) ifelse(sum(startsWith(x,"Desired Experience:"))==0,ifelse(sum(startsWith(x,"Experience"))==0,NA,x[startsWith(x,"Experience")]),x[startsWith(x,"Desired Experience:")])))
  ,location_tmp=html_nodes(indeed,".location , #p_68d09e88c4ff1c52 .location span")%>%html_text()
)
indeed_results$html=html
for(p in 1:length(page.links)-1){
  ...
  indeed_results_tmp=html_nodes(new.page,".result")
  html_tmp=lapply(indeed_results_tmp,function(x) ifelse(length(html_nodes(x,"h2 a"))==0,"No html",html_nodes(x,"h2 a")%>%html_attr('href')))
html=ifelse(html_tmp=="No html","No html",paste0(base,html_tmp))
  indeed_results_tmp=strsplit(html_text(indeed_results_tmp),"\n\n")

  indeed_results_tmp_df=data.frame(
    title=unlist(lapply(indeed_results_tmp, function(x) x[2])),
    company=gsub("\n","",unlist(lapply(indeed_results_tmp, function(x) ifelse(x[3]=="",x[4],x[3])))),
    experience=unlist(lapply(indeed_results_tmp, function(x) ifelse(sum(startsWith(x,"Desired Experience:"))==0,ifelse(sum(startsWith(x,"Experience"))==0,NA,x[startsWith(x,"Experience")]),x[startsWith(x,"Desired Experience:")])))
...
indeed_results=rbind(indeed_results,indeed_results_tmp_df)
Sys.sleep(2)
}

indeed_results=filter(indeed_results,html!="No html")
indeed_results=indeed_results[!duplicated(indeed_results),]


indeed_results$location=gsub("\\(.+|[0-9]","",as.character(indeed_results$location_tmp))
indeed_results=indeed_results[,-4]

desc=list()
for(i in 1:length(indeed_results$html)){
    cat("Moving to",i,"job\n")

  tmp=tryCatch({read_html(indeed_results$html[i])%>%html_text()},error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
  desc[i]=ifelse(length(tmp)==0,NA,tmp)
  Sys.sleep(2)
}


indeed_results$desc=unlist(desc)
indeed_results=filter(indeed_results,!is.na(desc))
write.csv(indeed_results,file="indeed_results1.csv")
